...
async function handleInvoicePaid(invoice: Stripe.Invoice) {
  ...
  console.log('[Webhook] Starting payment confirmation email process...')
  try {
    const { createServiceRoleClient } = await import('@/lib/supabase/server')
    const adminClient = createServiceRoleClient()
    
    console.log('[Webhook] Getting tenant and billing info...')
    ...
    console.log('[Webhook] Email data lookup:', {
      hasAdminEmail: !!adminEmail,
      adminEmail,
      hasTenant: !!tenant,
      tenantName: tenant?.company_name || tenant?.subdomain,
      hasBilling: !!billing,
      plan: billing?.plan
    })
    ...
    if (adminEmail && tenant && billing) {
      ...
      console.log(`[Webhook] Payment confirmation email sent to: ${adminEmail}`, emailResult)
    } else {
      console.log('[Webhook] Skipping email - missing data:', {
        hasAdminEmail: !!adminEmail,
        hasTenant: !!tenant,
        hasBilling: !!billing
      })
